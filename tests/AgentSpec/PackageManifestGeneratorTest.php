...
final class PackageManifestGeneratorTest extends TestCase
{
    public function testManifestExposesContractsClassesAttributesAndSidecars(): void
    {
        ...
        $this->assertSame(['FarewellInterface', 'GreeterInterface'], array_map(static fn ($c) => $c->shortName, $manifest->contracts));

        $classes = array_map(static fn ($c) => $c->shortName, $manifest->concreteClasses);
        ...
        $this->assertSame(
            ...
            array_map(static fn ($a) => $a->value, $manifest->attributeConventions),
        );
    ...
    private function loadDescriptor(): \Altair\AgentSpec\Model\PackageDescriptor
    {
