...
For a single entity, `$repository->save($entity)` is fine, as it persists and flushes in one call. For multi-entity transactions, drive the unit of work yourself:
...
In tests and during local development, bind `AttributeSchemaProvider` directly and let it recompile on each construction: convenient and slow, but the slowness rarely matters under `phpunit`.
...
The match is by name: the DTO property must be named like the relation. Only declared, matching relations are loaded; there is no over-fetching.
...
You can also use the hydrator directly on any array, such as a custom query result or a cache payload:
