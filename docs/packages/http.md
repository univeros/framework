...
The package does not handle HTTP/2 push, server-sent events, WebSockets, long-polling, or any stateful connection model. It does not include an HTML template engine beyond the optional `PhpViewFormatter`. It does not implement rate limiting beyond the spam-blocker referrer check; rate limiting is the responsibility of a reverse proxy or a dedicated middleware you add to the pipeline.
