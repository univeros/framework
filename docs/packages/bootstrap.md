...
What this package does *not* do: it does not run `composer install`, apply migrations, or boot a server for you, as those need the network and a chosen runtime, so the command prints them as next steps rather than performing them. It also does not generate frontend scaffolding or deployment manifests.
...
- **Skeleton template**: the project files, shipped inside the package at `resources/skeleton`. It travels with `univeros/bootstrap` when the monorepo is split, so the generator always finds it via `SkeletonGenerator::defaultSkeletonPath()`. The template uses the `App\` namespace and a `vendor/app` package name as placeholders. The skeleton also ships the lazy-loaded **Altair agent skill** at two paths so it's discoverable by any modern agent (#131): `.ai/skills/altair/SKILL.md` is the canonical, cross-agent Anthropic Agent Skills format (the convention Junie and Laravel Boost use), and `.claude/skills/altair/SKILL.md` is a byte-equal copy at Claude Code's project-skills path. The skill teaches shell-capable agents to drive the project through `bin/altair` and read `.agent/` manifests, instead of pulling every MCP tool's JSON schema into context every turn. A PHPUnit test in `tests/Bootstrap/SkeletonGeneratorTest.php` asserts the two copies stay byte-equal.
