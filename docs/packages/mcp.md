...
When you add a tool, mirror that last pattern: build the registry, drive a session over the in-memory transport, and assert the structured result, which exercises validation, resolution, and the protocol envelope together.
