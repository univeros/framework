...
If you're a shell-capable agent reading this and you've already opened the package source, you've taken the expensive path. The cheap path is the **Altair skill** (`src/Altair/Bootstrap/resources/skeleton/.claude/skills/altair/SKILL.md`, pre-installed in every project generated by `bin/altair new`) — it teaches the agent to drive the framework over bash and to read `.agent/` manifests for the API shape, without injecting every MCP tool's JSON schema into the context window on each turn.
