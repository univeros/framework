...
When the change set is too large to inline on one line (for example, a 200-file rector run), write the heavy diff to a snapshot and reference it (`->withSnapshotRef('snapshots/<id>.json')`) instead of bloating the log line.
