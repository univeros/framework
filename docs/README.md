...
- [Mcp](./packages/mcp.md) — a first-party Model Context Protocol server exposing the framework as 34 agent-callable tools over stdio/HTTP, so any MCP client can build, inspect, test, and ship an Altair API through tool calls.
