...
- [Logging](./packages/logging.md): PSR-3 application logging backed by Monolog, wired from `LOG_*` env vars. JSON-lines to stderr by default, so the framework's own log lines (and yours) stop hitting a `NullLogger`.
...
- [Observability](./packages/observability.md): `bin/altair observability:*`, OpenTelemetry-format runtime tracing + metrics. PSR-15 middleware records per-request spans, a JSONL log captures them locally, and an OTLP-JSON exporter forwards to any OTel Collector with no OTel SDK dependency. Tail / stats / one-shot export from CLI and MCP.
...
- [TestReporter](./packages/test-reporter.md): an AI-native PHPUnit 12 extension that emits a structured JSON report, mapping each failure back to the production source under test with structured diffs.
